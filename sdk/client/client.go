...
type Client struct {
	baseURL    string
	apiKey     string
	sourceID   string // Source ID for tracking which scanner/runner is pushing
...
type Config struct {
	...
	SourceID   string        `yaml:"source_id" json:"source_id"` // Registered source ID for audit trail
...
func (c *Client) doRequestOnce(ctx context.Context, method, url string, body []byte) ([]byte, error) {
	...
	// Add source ID header for audit trail
	if c.sourceID != "" {
		req.Header.Set("X-Rediver-Source-ID", c.sourceID)
	}
